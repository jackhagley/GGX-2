...
class MAP_Graph_TypeIssue2 extends Graph
{
  /*
  
   TypeIssue2 works out all the line sizes then displays them one by one
  ...
  MAP_Graph_TypeIssue2(Page p_, int x_, int y_, int w_, int h_)
  {
    super(p_, x_, y_, w_, h_);
    type = "MAP_Graph_TypeIssue2";
  ...
  void drawElement(PGraphics g)
  {
    ...
    float amount_of_space_for_column_titles = xStep/3;
    ...
    if (data!=null && !isNullOrBlank(sort_column)) 
    {
      ...
      for (int r = 0; r< data.getRowCount(); r++)
      {
        ...
        color ISSUE_COLOUR = lerpColor(COOL_GREY, HOT_ISSUE, colour_lerp_amount);
        //g.fill(BLACK);        
